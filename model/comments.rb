...
class Comments
  ...
  def self.get_comment(id_new)
    ...
    data = rawData.first
    Comments.new({ id: id_new, post_id: data['post_id'], comments_text: data['comments_text'],
                   attachment: data['attachment'], user_id: data['user_id'], date: data['date'] })
  end
  ...
  def self.get_all_comments(posts)
    ...
    rawData.each do |data|
      hash = { id: data['id'], post_id: data['post_id'], user_id: data['user_id'], date: data['date'],
               comments_text: data['comments_text'], attachment: data['attachment'] }
      comments << hash
    end
    if !comments.empty?
      {
  ...
  def self.get_comment_by_time
    ...
    if rawData.nil?
      ...
      comments = []
      rawData.each do |data|
        comment = Comments.new({ id: data['id'], post_id: data['post_id'], user_id: data['user_id'],
                                 date: data['date'], comments_text: data['comments_text'], attachment: data['attachment'] })
