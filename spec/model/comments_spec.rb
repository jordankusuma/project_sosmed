...
describe Comments do
  before :each do
    ...
    @comment = Comments.new({ id: 1, post_id: 2, comments_text: 'saya', attachment: 'a.png', user_id: 1,
                              date: '2021-08-17 17:04:54' })
    @comments_attach_nil = Comments.new({ id: 1, post_id: 2, comments_text: 'saya', attachment: nil, user_id: 1,
                                          date: '2021-08-17 17:04:54' })
    @comments = [
      { 'id': 1, 'post_id': 2, 'comments_text': 'saya', 'attachment': 'a.png', 'user_id': 1,
        'date': '2021-08-17 17:04:54' }
    ]
    @response = { 'id' => 1, 'post_id' => 2, 'comments_text' => 'saya', 'attachment' => 'a.png', 'user_id' => 1,
                  'date' => '2021-08-17 17:04:54' }
  end
    ...
    context 'when there are comments in post' do
      it 'return comments and status 200' do
        ...
        @comments.each do |data|
          hash = { id: data['id'], post_id: data['post_id'], user_id: data['user_id'],
                   date: data['date'], comments_text: data['post_text'], attachment: data['attachment'] }
          comment << hash
        end
  ...
  describe 'valid?' do
    ...
    context 'when not valid' do
      it 'return false when post_id is nil' do
        @comment = Comments.new({ id: 1, post_id: nil, comments_text: 'saya', attachment: 'a.png', user_id: 1,
                                  date: '2021-08-17 17:04:54' })
        expect(@comment.valid?).to eq(false)
      end
      it 'return false when user_id is nil' do
        @comment = Comments.new({ id: 1, post_id: 2, comments_text: 'saya', attachment: 'a.png', users: nil,
                                  date: '2021-08-17 17:04:54' })
        expect(@comment.valid?).to eq(false)
      end
      it 'return false when text is nil' do
        @comment = Comments.new({ id: 1, post_id: 2, comments_text: nil, attachment: 'a.png', user_id: 1,
                                  date: '2021-08-17 17:04:54' })
        expect(@comment.valid?).to eq(false)
      end
    end
